...
//       // Show the icon on user interaction
//     });
...
class _VideoPlayerWidgetState extends State<VideoPlayerWidget> {
  ...
  bool _showPlayPauseIcon =
      false; // Controls the visibility of the play/pause icon
  Timer? _hideIconTimer;
  bool isDisposed = false; // Flag to track if controller is disposed
  ...
  void _togglePlayPause() {
    if (isDisposed) return; // Prevent actions if controller is disposed
    ...
    return VisibilityDetector(
      ...
      child: _controller.value.isInitialized
          ? GestureDetector(
              onTap: () => _togglePlayPause(),
              child: Container(
                height: 16.5 / 9 * MediaQuery.of(context).size.width,
                color: Colors.black,
                child: Column(
